...
class BTCLNtoSol {
    ...
    async getCommitStatus(intermediary, data) {
        ...
        try {
            ...
            if (!escrowState.offerer.equals(intermediary) ||
    ...
    async isClaimable(intermediary, data) {
        ...
        try {
            const escrowState = await this.program.account.escrowState.fetch(this.getEscrowStateKey(Buffer.from(data.paymentHash, "hex")));
            if (!escrowState.offerer.equals(intermediary) ||
                !escrowState.claimer.equals(data.intermediary) ||
                !escrowState.mint.equals(data.token) ||
                !escrowState.initializerAmount.eq(data.amount) ||
    ...
    async createBOLT11PaymentRequest(address, amount, expirySeconds, url) {
        ...
        if (!new BN(decodedPR.millisatoshis).div(new BN(1000)).eq(amount))
    ...
    static generateLockingScript(csv_delta, hash, intermediaryKey, offererKey) {
        if (csv_delta < 0) {
    ...
    async createOnchainPaymentRequest(address, amount, url) {
        ...
        if (lockingScript.address !== jsonBody.data.address)
            throw new Error("Invalid payment request returned, amount mismatch");
        return {
            secret,
            hash: paymentHash,
    ...
    async getPaymentAuthorization(bolt11PaymentReqOrHash, minOut, url, address, abortSignal) {
        let paymentHash;
    ...
    async waitForIncomingPaymentAuthorization(bolt11PaymentReqOrHash, minOut, url, address, abortSignal, intervalSeconds, stateUpdateCbk) {
        if (abortSignal != null && abortSignal.aborted) {
            throw new Error("Aborted");
        }
        //const paymentHash = decodedPR.tagsObject.payment_hash;
        ...
        while (!abortSignal.aborted) {
            ...
            if (stateUpdateCbk != null) {
                if (result.txId != null)
                    stateUpdateCbk(result.confirmations, result.targetConfirmations, result.txId, result.vout, result.amount, result.swapFee, result.networkFee);
            }
            await timeoutPromise(intervalSeconds || 5);
        }
        throw new Error("Aborted");
    ...
    async isValidAuthorization(intermediary, data, timeout, prefix, signature, nonce) {
        ...
        const userAccount = await this.program.account.userAccount.fetch(this.getUserVaultKey(intermediary));
    ...
    async createPayWithAuthorizationIx(intermediary, data, timeout, prefix, signature, nonce) {
    ...
    async createPayWithAuthorizationTx(intermediary, data, timeout, prefix, signature, nonce) {
    ...
    async createClaimIx(intermediary, data, secret) {
        const expiryTimestamp = data.expiry;
        const currentTimestamp = Math.floor(Date.now() / 1000);
        console.log("[EVM.PaymentRequest] Expiry time: ", expiryTimestamp.toString());
    ...
    async createClaimTx(intermediary, data, secret) {
