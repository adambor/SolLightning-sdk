...
class BTCtoSolWrapper {
     ...
     */
    async create(amount, url) {
        if (!this.isInitialized)
            throw new Error("Not initialized, call init() first!");
        const result = await this.contract.createOnchainPaymentRequest(this.provider.wallet.publicKey, amount, url);
        const swap = new BTCtoSolSwap(this, result.address, result.secret, url, this.provider.wallet.publicKey, amount, result.swapFee.add(result.networkFee), result.generatedPrivKey, result.intermediaryBtcPublicKey, result.csvDelta);
        await swap.save();
